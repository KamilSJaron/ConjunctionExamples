...
pdf('Bazykin.pdf')
	plot(x,twidth(x,sqrt(0.5)),type='l',ylim=c(0,max(GradTable$width_l)),xlab = 's',ylab = 'w')
	points(width_l ~ s,pch = 20,data=GradTable)
	lines(x,twidth(x,sqrt(0.25)), col = 'red')
	points(width_l ~ s,pch = 20,data=GradTableLM, col = 'red')
dev.off()
